...
import { Home, Brain, CheckSquare, Archive } from 'lucide-react';
    ...
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-2 md:top-0 md:bottom-auto">
      <div className="max-w-4xl mx-auto flex justify-around items-center">
        ...
        <Link
          to="/commitments"
          className={`flex flex-col items-center p-2 ${
            isActive('/commitments') ? 'text-sage-600' : 'text-gray-600'
          }`}
        >
          <CheckSquare className="h-5 w-5" />
          <span className="text-xs mt-1">Commitments</span>
        </Link>
        <Link
          to="/archive"
          className={`flex flex-col items-center p-2 ${
            isActive('/archive') ? 'text-sage-600' : 'text-gray-600'
          }`}
        >
